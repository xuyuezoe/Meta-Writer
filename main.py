...
# ================================================================
# 兼容旧用法：不传命令行参数时，仍然默认运行这里指定的任务
#
# 目的：
#   让 `python main.py` / `python -m main` 在无参数时直接走正式 benchmark
#   主链路做烟雾验证，而不是回到早期的示例写作任务。
# ================================================================
TASK_NAME = "metabench_med_s001"
# ================================================================
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--task-name",
        type=str,
        help="指定已注册任务名，例如 scifi_story 或 metabench_med_s010",
    )
    parser.add_argument(
        "--task-id",
        type=str,
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="批量运行全部 benchmark 样本，并对每个样本的真实生成结果做评估",
    )
    parser.add_argument(
        "--list-tasks",
        action="store_true",
...
def _build_batch_summary(results: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    degraded_results = [
        item for item in results if str(item.get("status")) == "degraded"
    ]
    failed_results = [
        item for item in results if str(item.get("status")) == "failed"
    ]
