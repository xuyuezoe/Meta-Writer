"""MetaBench task derived from review article PMC13076180."""
...
def get_task_config() -> Dict[str, object]:
    """Return the PMC13076180-derived MetaBench task configuration."""

    spec = TaskSpec(
        task_id="med_pmc13076180",
        topic="Association between the atherogenic index of plasma and metabolic-associated/nonalcoholic fatty liver disease",
        domain="clinical evidence synthesis",
        target_words=7937,
        body_target_words=5500,
        expected_sections=6,
        practice_context="evidence-based clinical decision-making",
        organizer="systematic evidence synthesis framework",
        focus_points=[
                "Association between the atherogenic index of plasma and metabolic-associated/nonalcoholic fatty liver disease",
        ],
        extra_must_include=[
                "MAFLD",
                "NAFLD",
    ...
    config = build_main_task_config(
        spec,
        session_name="metabench_pmc13076180",
